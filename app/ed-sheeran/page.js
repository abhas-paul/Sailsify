...
const Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const storedSongs = localStorage.getItem('edSheeranSongs');
      ...
      if (storedSongs) {
        ...
        try {
          ...
          const edSheeranSongs = data?.['Ed Sheeran'] || [];

          localStorage.setItem('edSheeranSongs', JSON.stringify(edSheeranSongs));
          setSongs(edSheeranSongs);
          getTotalDuration(edSheeranSongs);
        } catch (error) {
    ...
    <section className="min-h-screen pb-25 bg-gradient-to-b mt-15 from-indigo-900 via-purple-800 to-blue-900 text-white">
      ...
      <section className="flex flex-col md:flex-row p-6 md:p-8 gap-6 items-center md:items-end">
        <section className="w-40 h-40 md:w-48 md:h-48 bg-gray-800 shadow-lg rounded-md overflow-hidden">
          <img
            src="https://i.pinimg.com/736x/be/1e/da/be1eda064654039297578561348b68d2.jpg"
            alt="Ed Sheeran"
        ...
        <section className="text-center md:text-left">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold my-2 md:my-4">Ed Sheeran</h1>
