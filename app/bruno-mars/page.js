...
const Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const storedSongs = localStorage.getItem('brunoSongs');
      ...
      if (storedSongs) {
        ...
        try {
          ...
          const brunoSongs = data?.Bruno || [];

          localStorage.setItem('brunoSongs', JSON.stringify(brunoSongs));
          setSongs(brunoSongs);
          getTotalDuration(brunoSongs);
        } catch (error) {
    ...
    <section className="min-h-screen pb-25 bg-gradient-to-b mt-15 from-indigo-900 via-purple-800 to-blue-900 text-white">
      ...
      <section className="flex flex-col md:flex-row p-6 md:p-8 gap-6 items-center md:items-end">
        ...
        <section className="text-center md:text-left">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold my-2 md:my-4">Bruno Mars</h1>
