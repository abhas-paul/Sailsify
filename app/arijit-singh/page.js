...
const Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const storedSongs = localStorage.getItem('arijitSongs');
      ...
      if (storedSongs) {
        ...
        try {
          ...
          const arijitSongs = data?.arijit || [];

          localStorage.setItem('arijitSongs', JSON.stringify(arijitSongs));
          setSongs(arijitSongs);
          getTotalDuration(arijitSongs);
        } catch (error) {
  ...
  const manageClick = (song) => {
    setCurrentSong({
      ...
      artist: song.artist || 'Arijit Singh',
    ...
    <section className="min-h-screen pb-25 bg-gradient-to-b mt-15 from-indigo-900 via-purple-800 to-blue-900 text-white">
      ...
      <section className="flex flex-col md:flex-row p-6 md:p-8 gap-6 items-center md:items-end">
        <section className="w-40 h-40 md:w-48 md:h-48 bg-gray-800 shadow-lg rounded-md overflow-hidden">
          <img
            src="https://image-cdn-ak.spotifycdn.com/image/ab67706c0000da84945b9170a805a07fca1ee80a"
            alt="Arijit Singh"
        ...
        <section className="text-center md:text-left">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold my-2 md:my-4">Arijit Singh</h1>
